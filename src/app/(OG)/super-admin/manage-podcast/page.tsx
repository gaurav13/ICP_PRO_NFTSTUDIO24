...
import {
  approvedPodcastMessage,
  rejectedPodcastMessage,
  ...
  async function sendEmail(
    ...
    if (user.ok) {
      if (user.ok[1].email.length != 0) {
        ...
        if (rejected) {
         let res= await sendArticleEmail({isApproving:false,email,name:username,articleTitle:title,contentType:"Podcast",reason});
if(res){
  toast.success(
    `Podcast rejected and email has been sent to user.`
  )
}
       
        } else {
          let res=await sendArticleEmail({isApproving:true,email,name:username,articleTitle:title,contentType:"Podcast"})
          if(res){
            toast.success(
              `Podcast Approved and email has been sent to user.`
            )
          }
        ...
        <td>
          ...
          {userAuth.userPerms?.articleManagement && (
            <div className='item-menu mt-1'>
              <Button
                onClick={() => {
                  openLink(article.isStatic?`${Podcast_STATIC_PATH+article.entryId}`:`/podcast?podcastId=${article.entryId}`);
                }}
