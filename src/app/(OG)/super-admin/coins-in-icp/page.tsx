...
export default function UserManagment() {
  ...
  const likeRewardSchema = object().shape({
    likeReward: number()
      ...
      .min(10, 'Coins Amount can not be less than 10').max(E8S, `Coins Amount can not be more than ${E8S}`)
      .test(
        'is-multiple-of-10',
  ...
  const handleCoinsInIcpUpdate = async () => {
    ...
    try {
      ...
      let updated = await userActor.update_NFT24Coin(
        newReward
      );
 
      logger(updated, 'UPPPPPPppp');
      toast.success('NFTStudio24 coins has been updated successfully');
      handleModalClose();
      getInitValues();
...
let amount=Math.ceil((1/(Number(likeReward)/E8S)))
    // let amount = (E8S/Number(likeReward));
    logger({likeReward,amount},"asdsadsadasdas");
                    ...
                    {({
                      ...
                      <FormikForm onSubmit={handleSubmit}>
                        ...
                        <Col xs='3' sm="2" md="1">
                          ...
                          <div className=' d-flex justify-content-center align-items-center' style={{width:"10px"}}>
                            <div>
                              =
                            </div>
                          </div>
                                  ...
                                  NFTStudio24 Coins 
                                    <Tippy
                                      content={
                                        <div>
                                          <p className='mb-0'>
                                            How many NFTStudio24 coins will be in one ICP.
                                          </p>
                                        </div>
                                      }
                                    >
