...
function HomeArticle({ article, small }: { article: any; small?: boolean }) {
  ...
  const getUser = async (userId: string) => {
    ...
    let res= await auth.actor.get_user_name(identity?._principal);
      let user:{image:[];name:string; designation:string;}| undefined=fromNullable(res);
     if(user){
                ...
                openArticleLink(
                  article[1].isPodcast
                    ? article[1].isStatic
                      ...
                      : `${
                          Podcast_DINAMIC_PATH + article[0] ?? 'noarticlefound'
                        }`
                    : article[1].isStatic
                    ? `${ARTICLE_STATIC_PATH + article[0]}`
                    : `${ARTICLE_DINAMIC_PATH + article[0] ?? 'noarticlefound'}`
                )
              }
              style={{
                overflowX: 'hidden',
      ...
      <main id='main' className='new-home logged-main'>
        ...
        <div className='main-inner home'>
          <div className='section scroll-anime anime-down' id='news'>
            <Row>
            <div className="col-xl-12 col-lg-12 col-md-12"><ins className="adsbygoogle" data-ad-layout="in-article" data-ad-format="fluid" data-ad-client="ca-pub-8110270797239445" data-ad-slot="3863906898"  style={{ display: 'block', textAlign: 'center' }}/></div>
