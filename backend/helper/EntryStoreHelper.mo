...
module EntryStoreHelper {
  ...
  public func searchListQuiz(array : [(Text, Quiz)], search : Text, startIndex : Nat, length : Nat, getModificationdate : (Key, Int) -> Int) : {
    entries : [(Text, Quiz)];
    amount : Nat;
  } {
    let searchString = Text.map(search, Prim.charToLower);
    ...
    var searchedQuizArray : [(Text, Quiz)] = Iter.toArray(sortedQuiz.entries());

    let compare = func((keyA : Text, a : Quiz), (keyB : Text, b : Quiz)) : Order.Order {
    ...
    var paginatedArray : [(Text, Quiz)] = [];
    let size = sortedEntries.size();
    let amount : Nat = size - startIndex;
    let itemsPerPage = 10;
    if (size > startIndex and size > (length + startIndex) and length != 0) {
      paginatedArray := Array.subArray<(Text, Quiz)>(sortedEntries, startIndex, length);
    } else if (size > startIndex and size > (startIndex + itemsPerPage)) {
      if (length != 0) {
        paginatedArray := Array.subArray<(Text, Quiz)>(sortedEntries, startIndex, amount);
      } else {
        paginatedArray := Array.subArray<(Text, Quiz)>(sortedEntries, startIndex, itemsPerPage);
  ...
  public func searchListServay(array : [(Text, Servay)], search : Text, startIndex : Nat, length : Nat, getModificationdate : (Key, Int) -> Int) : {
    entries : [(Text, Servay)];
    amount : Nat;
  } {
    let searchString = Text.map(search, Prim.charToLower);
    var sortedServay = Map.HashMap<Text, Servay>(0, Text.equal, Text.hash);
    ...
    var searchedServayArray : [(Text, Servay)] = Iter.toArray(sortedServay.entries());

    let compare = func((keyA : Text, a : Servay), (keyB : Text, b : Servay)) : Order.Order {
      let firstItemModificationDate = getModificationdate(keyA, a.creation_time);
      let secondItemModificationDate = getModificationdate(keyB, b.creation_time);
      if (firstItemModificationDate > secondItemModificationDate) {
        return #less;
    ...
    var paginatedArray : [(Text, Servay)] = [];
    let size = sortedEntries.size();
    let amount : Nat = size - startIndex;
    let itemsPerPage = 10;
    if (size > startIndex and size > (length + startIndex) and length != 0) {
      paginatedArray := Array.subArray<(Text, Servay)>(sortedEntries, startIndex, length);
    } else if (size > startIndex and size > (startIndex + itemsPerPage)) {
      if (length != 0) {
        paginatedArray := Array.subArray<(Text, Servay)>(sortedEntries, startIndex, amount);
      } else {
        paginatedArray := Array.subArray<(Text, Servay)>(sortedEntries, startIndex, itemsPerPage);
