...
shared ({ caller = initializer }) actor class () {
  ...
  public shared ({ caller }) func addActivity(user : UserId, target : Text, activityType : ActivityType, title : Text) : async Bool {
    ...
    Debug.print(debug_show ("Tried to add comment from this caller", caller));
    assert Principal.isController(caller);
    Debug.print(debug_show ("we did it "));
  ...
  public shared ({ caller }) func addComment(inputComment : InputComment, userCanisterId : Text, article : Text,title:Text,entryType:Text) : async Result.Result<(Comment, Text), Text> {
    assert not Principal.isAnonymous(caller);
